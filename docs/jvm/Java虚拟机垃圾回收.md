...
```
bashReferenceCountingGC objA = new ReferenceCountingGC();
...
**一般情况下，应尽量避免使用它，甚至可以忘掉它。** ### 3、HotSpot虚拟机中对象可达性分析的实现
...
分配担保在以后讲解垃圾收集器执行规则时再详解； ### 3、标记-整理算法
